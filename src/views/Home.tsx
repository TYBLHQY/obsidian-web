import { exampleMarkdown } from "@/assets/md/markdownExample";
...
const Home = defineComponent({
  ...
  setup() {
    const { markdown, ast, wordCount, charCount, lineCount, setMarkdown } = useMarkdown(exampleMarkdown);
    const { extractHeadings, extractLinks, extractImages } = useMarkdownParser();

    const headings = computed(() => {
      return extractHeadings(ast.value);
    });

    const links = computed(() => {
      return extractLinks(ast.value);
    });

    const images = computed(() => {
      return extractImages(ast.value);
    });
